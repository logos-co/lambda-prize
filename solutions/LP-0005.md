...
Two verification paths: **on-chain** (LEZ SPEL gating program) and **off-chain** (standalone verifier library, partial — Logos Messaging integration not yet wired).
...
- [ ] **Off-chain path via Logos Messaging** — `verifier-lib` provides the full off-chain verification logic; Logos Messaging transport integration is not yet wired. The library can be consumed by any caller; connecting it to Logos Messaging requires adding the send/receive layer.
...
Off-chain: `verify_attestation` returns an `anyhow::Error` with a message for each failure case, not a panic. Private account data does not appear in error messages.

...
- **On-chain**: A LEZ verifier contract gates on-chain actions (governance votes, access grants, etc.).
...
Once generated, that proof is a self-contained, verifiable credential. It can be submitted to a LEZ contract for on-chain gating, or broadcast over Logos Messaging for off-chain gating — for example, to gain entry to a token-gated chat group. The verifier (on-chain contract or off-chain recipient) needs only the proof and the public parameters; it learns nothing else about the prover.
...
- [ ] The proof is bound to a specific context (e.g., a contract address, group ID, or application identifier) to prevent replay across different gates.
...
- [ ] **On-chain path**: A LEZ verifier contract accepts and verifies the proof, gating at least one on-chain action.
...
- Public repository with all circuit code, LEZ verifier contract, off-chain verifier library, and client-side tooling under MIT or Apache-2.0.

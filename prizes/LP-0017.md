# LP-0017: Anonymous Forum with Threshold Moderation and Membership Revocation
...
**Anonymous posting**: Members register with a private identity. Each post carries a proof that the author is a registered, non-revoked member — without revealing which member. Posts from the same author are unlinkable to outside observers.
...
One concrete approach combines homomorphic encryption and Shamir's Secret Sharing (RLN is one realisation of this pattern):

- Each member registers with a commitment that encodes a secret split into shares according to the revocation threshold K.
- Each post carries a membership proof encrypted homomorphically: anyone can verify the proof is valid without decrypting it, and the same member's posts remain unlinkable.
- A moderation event triggers a coordinated threshold decryption by N-of-M moderators, producing one Shamir share of the author's nullifier. The share is recorded alongside the moderation certificate.
- Once K shares have been accumulated across K moderation events, they are sufficient to reconstruct the nullifier. That nullifier is submitted on-chain to slash the member.

Applicants are not required to follow this approach. Alternative cryptographic constructions that satisfy the goals above are in scope.

## Success Criteria

- [ ] A member can register in a forum instance with a stake and subsequently publish posts with valid, anonymous, proofs of membership. Posts from the same member are unlinkable to a third-party observer.
...
- [ ] A working Logos mini-app (runnable inside the Logos app), built using the library without modifying it. The app allows anyone to create a new forum instance, post, moderate it, and set moderators. The specific forum functionality (text posts, image board, comments, reactions, etc.) is at the applicant's discretion. The app demonstrates the full moderation lifecycle: instance creation, registration, publishing content, moderation, and revocation.

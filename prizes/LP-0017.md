...
The approach here separates two concerns: proving membership (done via an encrypted proof attached to every post) and revealing identity (only possible through coordinated moderator action). Because the proof is encrypted, no single moderator — and no outside observer — learns who wrote a post. Because decryption requires N-of-M moderators to agree, no single moderator can unilaterally act. And because each decryption only reveals one share of the member's secret, the member's identity is only exposed once they have been moderated enough times to exhaust their threshold. At that point, the evidence on-chain is sufficient for anyone to complete the slash — no trusted party needed.

...
- `contract.query(program_id, params)` — reads state from a LEZ contract.
- `contract.call(program_id, instruction, params)` — submits a transaction to a LEZ contract; subject to spending threshold.
- `contract.deploy(binary_path)` — deploys a compiled LEZ program binary to the network and returns the program ID.
...
- **On-chain event alerter**: agent monitors a LEZ contract or account for state changes and notifies the owner via Logos Messaging.
